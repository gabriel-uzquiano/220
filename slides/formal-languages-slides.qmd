---
title: "formal languages"
subtitle: "PHIL 220. Introduction to Logic"
author: "Gabriel Uzquiano"
format:
  revealjs:
    theme: [default, custom.scss]
    output-file: formal-languages-slides.html
    slide-number: c/t
    chalkboard: true          # press B for a blank board, C to draw on a slide
    preview-links: auto       # links open in an overlay, without leaving the deck
    history: false
    transition: none
    center: false
    width: 1100
    height: 750
    margin: 0.06
    hash-type: title
    menu:
      side: left
---

# formal languages {.divider}

We have emphasized that validity is a matter of form. 

. . .

We have focused on forms that combine basic statements with the help of connectives like 'or', 'and', 'not', 'if ... , then ...'.

. . .

Notice the use of *quotation marks* ... 

# use and mention {.divider}

Quotation marks are used to distinguish between use and mention.

## a distinction between use and mention

::: {.columns}
::: {.column width="48%"}
Los Angeles has many cities in its metropolitan area.

::: {.fragment .caption}
We predicate something of a *city*, not a *linguistic expression*.
:::
:::

::: {.column width="48%"}
'Los Angeles' consists of two words.

::: {.fragment .caption}
We predicate something of a *name*, which is itself a *linguistic expression*.
:::
:::
:::



::: {.incremental}
- One statement **uses** a name for a city in order to denote a city.

- The other statement **mentions** a name for a city in order to predicate something of the linguistic expression.

- Quotation marks indicate that a given expression is being mentioned and not used.
:::


## practice

Which statements below are true?

```{.QualitativeProblem .MultipleSelection options="check" submission="none"}
A.
| England is part of 'New England'
|*'England' is part of 'New England'
|'England' is part of New England
```


```{.QualitativeProblem .MultipleSelection options="check" submission="none"}
B.
| 'England' is not part of 'New England'
|* England is not part of 'New England'
|* England is not part of New England
```




# form and meaning {.divider}

Which words does the form depend on?

## whence a valid form

::: {.argument}
1. $p$ and $q$

::: {.conclusion}
2. Therefore, $q$
:::
:::

. . .

This is a valid argument form because of what **'and'** means. Replace 'and'
with 'or' and we no longer have a valid form:

. . .

::: {.columns}
::: {.column width="48%"}
::: {.argument}
1. $p$ or $q$

::: {.conclusion}
2. Therefore, $q$
:::
:::
:::

::: {.column width="48%"}
::: {.argument}
1. Snow is white or black.

::: {.conclusion}
2. Therefore, snow is black.
:::
:::
:::
:::


## a truth table


<div style="display:flex; justify-content:center; margin-top:1rem;">
<table style="border-collapse:collapse; font-size:0.95em;">
  <thead>
    <tr>
      <th style="text-align:center; padding:0.35rem 0.9rem; border-bottom:2px solid #444;">$p$</th>
      <th style="text-align:center; padding:0.35rem 0.9rem; border-right:4px double #444; border-bottom:2px solid #444;">$q$</th>
      <th style="text-align:center; padding:0.35rem 0.9rem; border-bottom:2px solid #444;">$p$ and $q$</th>
      <th style="text-align:center; padding:0.35rem 0.9rem; border-bottom:2px solid #444;">$q$</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style="text-align:center; padding:0.3rem 0.9rem;">T</td>
      <td style="text-align:center; padding:0.3rem 0.9rem; border-right:4px double #444;">T</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">T</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">T</td>
    </tr>
    <tr>
      <td style="text-align:center; padding:0.3rem 0.9rem;">T</td>
      <td style="text-align:center; padding:0.3rem 0.9rem; border-right:4px double #444;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
    </tr>
    <tr>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem; border-right:4px double #444;">T</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">T</td>
    </tr>
    <tr>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem; border-right:4px double #444;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
      <td style="text-align:center; padding:0.3rem 0.9rem;">F</td>
    </tr>
  </tbody>
</table>
</div>

. . .

No matter what statements we choose for $p$ and $q$, no argument instance will take us from *true* premises to a *false* conclusion.



## four basic connectives

| | | |
|---|---|---|
| $\neg$ | negation | it is not the case that |
| $\wedge$ | conjunction | and |
| $\vee$ | disjunction | or |
| $\to$ | the conditional | if &hellip;, then &hellip; |

: {.symbols tbl-colwidths="[8,26,66]"}

. . .

Propositional logic will focus on forms obtained from basic propositional letters $p$, $q$, $r$, etc and the four connectives listed above.



# how to specify a language  {.divider}

Syntax, semantics, pragmatics.

## three aspects of a language

::: {.incremental}
- **Syntax** 
  
    - What is the basic vocabulary from which to build the expressions of the language?
    - Which combinations of symbols count as grammatical sentences of the language.

- **Semantics** 

    - How should the language be interpreted?
    - What do the sentences of the language mean?

- **Pragmatics** 

    - What do speakers *convey* with an utterance of a sentence?
:::

. . . 

Only syntax and semantics will be relevant for a formal language.


# why a formal language? {.divider}

There are at least two reasons 

## reason one: discount pragmatic effects

Do these sentences differ in truth conditions?

> I felt ill, and I took some syrup

> I took some syrup, and I felt ill

. . .

In a formal language, we stipulate that order makes no difference to the truth conditions of a conjunction: $p \wedge q$ is true if, and only if, $q \wedge p$ no matter how we interpret $p$ and $q$.


## reason two: structure is not transparent in English {.smaller}

::: {.columns}
::: {.column width="46%"}
::: {.fragment}
**premises**

1. Some Rolls Royce is better than any Audi.
2. Some Audi is better than any Honda.

::: {.conclusion}
**conclusion**

3. Some Rolls Royce is better than any Honda.
:::
:::
:::

::: {.column width="50%"}

::: {.fragment}
**premises**

1. Some gift card is better than nothing.
2. Nothing is better than any fine Swiss watch.

::: {.conclusion}
**conclusion**

3. Some gift card is better than any fine Swiss watch.
:::
:::

:::
:::

. . .

One may be tempted to discern the same superficial form:

::: {.fragment}
1. Some $A$ is better than any $B$.
2. Some $B$ is better than any $C$.

::: {.conclusion}
3. Some $A$ is better than any $C$.
:::
:::

. . .


That would be a **mistake**. The two arguments do **not** share a valid form. 

Quantificational logic will enable to distinguish two different forms, one valid and one invalid.




## Structural ambiguity

> There is gas in the tank or the car will start if you turn the ignition.

. . .

::: {.incremental}
- $\text{there is gas in the tank}$ **or** $[\text{you turn the ignition} \Rightarrow \text{the car starts}]$ 
- $[\text{there is gas in the tank}$ **or** $\text{you turn the ignition}] \Rightarrow \text{the car starts}$ 
:::

. . .

Unlike English, the formal language will not tolerate structural ambiguities.


# the language of propositional logic {.divider}

## vocabulary {.smaller}

The vocabulary of propositional logic contains three types of symbols:

Propositional Variables

:   These are the lowercase letters $p$, $q$, $r$, $s$, and $t$ with or without numerical subscripts:
   $$
   p,  q, r, s, t
   $$

Connectives

:   These are formal counterparts of 'not', 'or', 'and', and 'if $\dots$, then $\dots$':
   $$
   \neg, \vee, \wedge, \to
   $$

Parentheses

:   There are two parentheses:
   $$
   ), (
   $$

**Nothing else** is a symbol of the language.

## grammar

Which expressions count as a formula of propositional logic?

:::{.incremental}
1. All propositional variables are *formulas*.\
2. If $\varphi$ and $\psi$ are *formulas*, then each of
$$
\neg  \varphi, (\varphi \wedge \psi), (\varphi \vee \psi), (\varphi \to \psi)
$$
is a *formula*.\
3. Nothing else is a *formula*.
:::

## example

```{.FormulaChecker card="tree"}
(p ∧ (q → ¬r))
```


## Next time



::: {.aside}
The slides cover material from [Chapter 1](https://gabriel-uzquiano.github.io/logic/reason.html#reason) of [Introduction to Logic](https://gabriel-uzquiano.github.io/logic/).
:::