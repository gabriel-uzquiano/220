...
## an apparent contrast with 'or' {.smaller}
...
## an apparent contrast with 'if ... then ...' {.smaller}
...
3. Draw additional columns for the relevant subformulas, working from the
   ...
   ```{.TruthTable card="table" height="500px"}
(p -> ((q /\ ~r) -> q))
```
...
4. Fill each additional column according to the truth-value rule for its
   ...
   ```{.TruthTable card="table" solved="true" height="500px"}
(p -> ((q /\ ~r) -> q))
```
...
It is false in the second row of the truth table.
:::

::: {.column width="70%"}
```{.TruthTable solved="true" mode="tautology" height="490px"}
(p -> (p -> q))
```
:::
:::
...
It is true in all rows of the truth table.
:::
...
They agree in truth value in all rows of the table.
:::

::: {.column width="70%"}
```{.TruthTable solved="true"  mode="equivalence" height="490px"}
(p -> q)  
(~q -> ~p)
```
:::
...
They differ in truth value in the second and third row of the table.
:::
...
All three are true in the last two rows of the truth table.
:::

::: {.column width="70%"}
```{.TruthTable solved="true"  mode="consistency" height="490px"}
(p -> q)  
(~q -> ~p)
~ p
```
...
No row verifies all four formulas.
:::
...
**No row makes the premises true and the conclusion false**.
:::
...
**The second row makes the premises true and the conclusion false**.
:::
